from furniture import model, repository
from furniture.test.fixture import session, in_memory_db
...
def test_repository_can_save_a_batch(session):
    ...
    rows = list(session.execute(
        text('SELECT * FROM "batches"')
    ))
    assert rows == [(1, "batch1", "RUSTY-SOAPDISH", 100, None)]


def insert_order_line(session):
    session.execute(
        text('INSERT INTO order_lines (ref, sku, qty) VALUES ("order1", "GENERIC-SOFA",12)')
    )
    [[orderline_id]] = session.execute(
        text('SELECT id FROM order_lines WHERE ref =:ref AND sku =:sku'),
             dict(ref="order1", sku="GENERIC-SOFA")
    )
